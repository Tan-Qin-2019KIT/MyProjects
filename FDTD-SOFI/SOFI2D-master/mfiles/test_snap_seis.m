%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%---this code is modified from SOFI3D by Tan Qin in 05.04.2019
%---it can be used to display the snapshot obtained by SOFI2D and SOFI3D

%---script for the visualization of snapshots gained from the SOFI2D simulation
%---most parameters are as specified in SOFI2D parameter-file, e.g. sofi2D.json
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nx=400; ny=100; nz=100; % basic grid size; ny=vertical
...
file_inp1=(['../par/snap/' filename '.bin.div']);
% Input file2 (snapshot file2)
file_inp2=(['../par/snap/' filename '.bin.curl']);
% Model file (for single display or contour plot ontop of snapshot)
file_mod=(['../par/model/' filename '.rho']);
...
file_out=(['../par/snap/' filename '_pic']);
...
caxis_value_1=5e-4;
caxis_value_2=5e-4; % only used if num_switch=2
...
if (cont_switch==1) | (type_switch==1)
    ...
    if model_switch==1                
        mod_data=reshape(mod_data,ny,nx);
        fclose(fid_mod);
    else
...
if image_switch==1
    ...
    set(h1,'Position',[scrsz(3)*1/4 scrsz(4)*1/4 scrsz(3)*1/2 scrsz(4)*1/2]);
