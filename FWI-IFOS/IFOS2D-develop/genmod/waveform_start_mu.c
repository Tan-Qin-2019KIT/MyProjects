...
void model_elastic(float  **  rho, float **  pi, float **  u){
	...
	float Rho, Vp, Vs, Vpnm1, x, y, undf, r, k;
	...
	const float r2 = 5.5;
        ...
        FP1=fopen("/lustrefs/homes/koed/scratch/DENISE/par/mu_start/waveform_sphere_k_it_198.bin","rb");
	...
	for (i=1;i<=NXG;i++){
		for (j=1;j<=NYG;j++){
			...
			if ((POS[1]==((i-1)/NX)) && 
				...
				u[jj][ii]=Vs*Vs*Rho;
				rho[jj][ii]=Rho;
				pi[jj][ii]=Vp*Vp*Rho - (4.0/3.0)*(Vs*Vs*Rho);
			}
		}
	}	
