...
int main(int argc, char **argv){
	...
	int n, maxcpu=0, m=0, wsam=0, l, b=0, c;
	...
	if ((outfile=fopen(outname,"w+"))!=NULL) /* fprintf(stderr, "\t File %s opend for writing. ... \n",outname) */;
	else {
	...
	if((infiles=fopen(inname,"r+"))!=NULL){
		...
		if (nsamp!=msamp) fprintf(stdout,"Warning: Number of samples in SEG-Y header and 1st trace header differ?!\n");
