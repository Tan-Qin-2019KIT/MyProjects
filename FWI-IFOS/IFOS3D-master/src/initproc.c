 ...
 * Dividing the 3-D FD grid into domains and assigning the PEs to these domains.
 * 
 * written by T. Bohlen 
...
void initproc(void)	{
	...
	if (POS[2] == NPROCY-1) INDEX[4]=MYID+NPROCX-(NPROCX*NPROCY);	only necessary for periodic boundary condition */
	
	if (POS[1] == 0)        INDEX[1]=MPI_PROC_NULL;        	  
	if (POS[1] == NPROCX-1) INDEX[2]=MPI_PROC_NULL;          	 
	if (POS[3] == 0)        INDEX[5]=MPI_PROC_NULL;	 
	if (POS[3] == NPROCZ-1) INDEX[6]=MPI_PROC_NULL; 	 
	if (POS[2] == 0)        INDEX[3]=MPI_PROC_NULL; 	 
	if (POS[2] == NPROCY-1) INDEX[4]=MPI_PROC_NULL;	
